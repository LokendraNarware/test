...
if (isset($_POST['go'])) {
    $contact = (isset($_POST['contact']) && $_POST['contact'] == "On");
    $backlinks = (isset($_POST['backlinks']) && $_POST['backlinks'] == "On");

    if ($contact) {
        $className = "EasyContact";
        $name = "Easy Contact Page";
        $version = "1.0";
        $marketId = 2;

        $path = realpath("../includes/plugins/$className/");

        if (!$path) die("Missing plugin $className");
        ...
        $studio->sql->query("INSERT INTO plugins (name, directory, market_id, enabled, version) VALUES ('$name', '$className', $marketId, 1, '$version')");
    }
    if ($backlinks) {
        $className = "Backlinks";
        $name = "Backlink Tool Pack";
        $version = "1.0";
        $marketId = 1;
        ...
        <table cellspacing="0" cellpadding="0">
            <tr valign="top">
                <td width="225px">
                    ...
                    <nav>
                        <ul>
                            ...
                            <li class="active"><a>Install plugins</a></li>
                            <li><a>Updates</a></li>
                ...
                <td>
                    ...
                    <p>We've packaged this installer with some free plugins. Choose the plugins you'd like to enable.</p>

                    <form action="" method="post">
                        <label for="cb1"><input id="cb1" type="checkbox" name="backlinks" value="On" checked> <strong>Backlinks Tool Pack</strong> &mdash; Adds four tools for finding backlinks using your free SEO Profiler account.</label>

                        <br><br>
                        <label for="cb2"><input id="cb2" type="checkbox" name="contact" value="On" checked> <strong>Easy Contact Page</strong> &mdash; Adds a contact page.</label>

                        <br><br>
                        <input type="submit" class="btn blue" value="Continue" name="go">
                    </form>
                </td>
            </tr>
        </table>
