...
if (!$api->isAuthorized()) {
    $error = "You have not activated your copy of Studio, so this feature is unavailable.";
}
...
if (isset($_POST['email'])) {
    ...
    if ($email != "") {
        if ($url != "") {
            try {
                $success = $api->enableGoogle($url, $email);
                if ($success) {
                    ...
                    $error = "Failed to enable the network.";
                }
            }
            catch (Exception $e) {
                $error = $e->getMessage();
        ...
        else $error = "Please enable JavaScript.";
    }
    else $error = "You must enter a valid email to join the network.";
}
...
<body>
    <div class="panel">
        <table cellspacing="0" cellpadding="0">
            <tr valign="top">
                <td width="225px">
                    ...
                    <nav>
                        <ul>
                            ...
                            <li class="active"><a>Google network</a></li>
                ...
                <td>
                    <h1>Google Network</h1>
                    <?php if ($error != "") echo "<div style=\"margin: 10px 0;\" class=\"error\">$error</div>"; ?>

                    <p>This product scrapes data from Google in order to power some of its tools. As a result, Google may sometimes block your IP address, especially if you're on a shared server. This is a free service which unblocks you from Google.</p>

                    <p><strong>It is <span style="color: #F44336;">very important</span> that you read the following info if you want to join the network.</strong></p>
                    <iframe class="termsframe" src="../resources/eulas/google.html"></iframe>

                    <p style="margin-top: 15px;">Want to join? Enter your email so we can alert you if the terms above ever change.</p>
                    <form action="" method="post">
                        <input type="text" name="email" placeholder="Email address" class="fancy" style="margin-bottom: 10px;">
                        <input type="hidden" id="url" name="url">

                        <p style="margin: 20px 0 8px;">By clicking Enable, you declare that you have read the above terms understand how the service works.</p>
                        <input type="submit" value="Enable" class="btn blue"> <a class="btn" href="account.php">Skip</a>
                    </form>
                </td>
            </tr>
        </table>
    ...
    <script>
        ...
        loc = loc.substring(0, loc.indexOf("/install/"));
