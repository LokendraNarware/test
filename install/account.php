    ...
    if ($password != $password2) {
        ...
        if (strlen($password) < 4) {
            ...
            if ($email == "") {
                ...
                $adminGroup = (int)$studio->getopt("default-group") + 1;
                $time = time();
                ...
                if (!$password) die("Failed to crypt password.");

                $p = $studio->sql->prepare("INSERT INTO accounts (email, password, timeLastLogin, timeCreated, groupId) VALUES (?, ?, $time, $time, $adminGroup)");
