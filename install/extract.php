...
$error = "";
$installto = dirname(dirname(__FILE__));

if (!file_exists("studio.zip")) {
...
function extractFiles() {
    global $error, $installto;

    $zip = new Zip;
    $zip->read_zip("studio.zip");

    if (!is_writable($installto)) {
        $error = "$installto is not writable. Please fix permissions and try again.";
        return;
    }

    foreach ($zip->dirs as $dir) {
        $path = $installto . DIRECTORY_SEPARATOR . $dir;

        if (!file_exists($path)) {
            if (!mkdir($path, 0777, true)) {
                $error = "Failed to create directory $path. No permissions.";
                return;
            }
        }
    }
    foreach ($zip->files as $file) {
        $path = $installto . DIRECTORY_SEPARATOR . $file['dir'] . DIRECTORY_SEPARATOR . $file['name'];
        $size = file_put_contents($path, $file['data']);
        if ($size === false) {
        ...
        <table cellspacing="0" cellpadding="0">
            <tr valign="top">
                <td width="225px">
                    ...
                    <nav>
                        <ul>
                            ...
                            <li class="active"><a>File extraction</a></li>
                ...
                <td>
                    ...
                    <p>The application files will now be extracted. Please confirm that the directory shown below is where you would like to extract the files into.</p>
                    <p><strong>We will create the following files and dirs.</strong> Please make sure it is the correct directory.</p>
                    ...
                    <ul>
                        ...
                        foreach ($zip->dirs as $dir) {
                            if (stripos($dir, "/") !== false) continue;
                            echo "<li>$installto" . DIRECTORY_SEPARATOR . "$dir</li>";
                        }
                        foreach ($zip->files as $file) {
                            if ($file['dir'] != "") continue;
                            $fe = "";
                            if (file_exists("$installto/".$file['name'])) $fe = "<span style='color: red'>(already exists, will be overwritten)</span>";
                            echo "<li>$installto" . DIRECTORY_SEPARATOR . $file['name'] . " $fe</li>";
                        }
                        ?>
                    </ul>

                    <div style="margin: 10px 0 0;">
                        <a class="btn blue" href="?go">Extract</a>
                    </div>
                </td>
            </tr>
        </table>
